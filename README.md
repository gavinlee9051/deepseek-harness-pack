...
- 会话归档（归档/恢复/永久删除会话）：与解锁补丁一同自动应用（`patches/`，按 dsh 版本选择，源自 gavinlee9051/dsh-modern-skin）
...
| `patches/` | 核心补丁定义（`archive-core-rc2.mjs` / `archive-core-0.1.5.mjs`） |
...
会话可归档 / 恢复 / 永久删除。实现为对 dsh 核心若干包的编译产物做锚点替换
（`patches/`），Linux 由 `patch-archive.sh`、Windows 由 `win/dsh.ps1` 在**每次启动**时
幂等应用（含升级后自动重打）。

- **版本绑定**：补丁按 dsh 版本选择，已验证 `0.1.1-rc.2`（`archive-core-rc2.mjs`）
  与 `0.1.5-rc.1`（`archive-core-0.1.5.mjs`，0.1.5 上游已内置归档，补丁补齐恢复与删除）。
  `status` 会显示「归档补丁」状态；其它版本启动脚本会**自动跳过**并提示（避免误改不兼容的代码），
  直到补丁随新版本锚点更新。
